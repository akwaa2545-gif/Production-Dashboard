...
function shortTaMtdSeries(value) { const match = String(value || '').match(/\b([A-Z]+)\s+series\s+([^\s]+)\s+case\b/i); return match ? `${match[1].toUpperCase()} ${match[2]}` : value; }
async function loadParameterSeries() { const product = byId('parameterProduct').value; const serie = byId('parameterSerie'); if (!product) { serie.replaceChildren(new Option('Select product first', '')); serie.disabled = true; return; } byId('parameterStatus').textContent = ''; try { const dataset = product === 'TA' ? 'ta-yield' : selectedDataset(); const options = await request(`/api/options?${new URLSearchParams({ dataset, product })}`); const values = product === 'TA' ? [...new Set(options.serie.map(shortTaMtdSeries))].sort() : options.serie; serie.replaceChildren(new Option('Select serie', ''), ...values.map((value) => new Option(value, value))); serie.disabled = false; const dashboardSelection = selectedSeries(); if (dashboardSelection.length === 1 && values.includes(dashboardSelection[0])) serie.value = dashboardSelection[0]; fillParameterSetting(); } catch (error) { byId('parameterStatus').textContent = error.message; } }
