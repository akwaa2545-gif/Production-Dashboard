...
const taChartGroup = (line) => /^PS/i.test(String(line || '')) ? 'Standard Prod' : /^FP/i.test(String(line || '')) ? 'Facedown Prod' : /^GPS\s*P2/i.test(String(line || '')) ? 'GPS Prod' : String(line || 'Other TA series');
...
function applyWipInsightVisibility() { byId('toggleWipFlow').checked = wipInsightVisibility.flow; byId('toggleYield').checked = wipInsightVisibility.yield; byId('flowInsightPanel').hidden = !wipInsightVisibility.flow; byId('yieldInsightPanel').hidden = !wipInsightVisibility.yield; }
function setWipInsightVisibility(next) { wipInsightVisibility = { ...wipInsightVisibility, ...next }; localStorage.setItem(wipInsightVisibilityKey, JSON.stringify(wipInsightVisibility)); applyWipInsightVisibility(); }
...
async function loadCellComments() { commentsByCell.clear(); const scope = commentScope(); if (!commentsEnabled || !scope.product) return; const params = new URLSearchParams({ ...scope, startDate: byId('startDate').value, endDate: byId('endDate').value }); const comments = await request(`/api/comments?${params}`); comments.forEach((comment) => commentsByCell.set(commentKey({ ...comment, reportingDate: String(comment.reportingDate).slice(0, 10) }), comment)); }
...
async function loadCommentLog() { const rows = byId('commentLogRows'); const status = byId('commentLogStatus'); if (!commentsEnabled) { status.textContent = 'Cell comment storage has not been configured.'; rows.innerHTML = '<tr><td colspan="8">Comment storage is unavailable.</td></tr>'; return; } status.textContent = 'Loading comments...'; try { const comments = await request('/api/comments/all'); rows.innerHTML = comments.map((comment) => `<tr><td>${escapeHtml(commentDate(comment.reportingDate))}</td><td>${escapeHtml(comment.product)}</td><td>${escapeHtml(comment.serie)}</td><td>${escapeHtml(comment.pn || '-')}</td><td>${escapeHtml(comment.process || '-')}</td><td class="comment-log-text">${escapeHtml(comment.commentText)}</td><td>${escapeHtml(comment.createdBy)}</td><td>${escapeHtml(commentTimestamp(comment.updatedAt || comment.createdAt))}</td></tr>`).join('') || '<tr><td colspan="8">No active comments.</td></tr>'; status.textContent = comments.length ? `${comments.length} active comment${comments.length === 1 ? '' : 's'}.` : ''; } catch (error) { rows.innerHTML = '<tr><td colspan="8">Comments could not be loaded.</td></tr>'; status.textContent = error.message; } }
...
function showView(view) { const parameters = view === 'parameters'; const comments = view === 'comments'; const model = view === 'model'; const scCalculationLog = view === 'sc-yield-log'; const taCalculationLog = view === 'ta-yield-log'; const calculationLog = scCalculationLog || taCalculationLog; const scYieldParameters = parameters && currentConfig.dataset === 'yield'; const scLogView = ensureScYieldLogView(); const taLogView = ensureTaYieldLogView(); byId('dashboardView').hidden = parameters || comments || model || calculationLog; byId('parameterView').hidden = !parameters || scYieldParameters; byId('scYieldParameterView').hidden = !scYieldParameters; scLogView.hidden = !scCalculationLog; taLogView.hidden = !taCalculationLog; byId('commentView').hidden = !comments; byId('dataModelView').hidden = !model; document.querySelectorAll('.app-tab').forEach((button) => { const active = button.dataset.view === view; button.classList.toggle('active', active); button.setAttribute('aria-pressed', String(active)); }); if (parameters) { if (scYieldParameters) renderScYieldTargetParameters(latestScYieldData); else { if (!byId('parameterProduct').value && byId('product').value) byId('parameterProduct').value = byId('product').value; loadParameterSeries(); renderSavedParameters(); } } if (scCalculationLog) renderScYieldCalculationLog(); if (taCalculationLog) renderTaYieldCalculationLog(); if (comments) loadCommentLog(); if (model) renderDataModel(currentConfig.dataModels); }
...
function renderData(data, chartData, mtdData = data) {
  ...
  const zeroSeries = currentConfig.dataset === 'closed' && !groupedByPn && showZeroSeries ? [...byId('serie').options].map((option) => option.value).filter(Boolean) : [];
...
function renderTaYieldGroupTendencyCharts(rows, buckets) {
  ...
  if (!series.length || !buckets.length) { holder.innerHTML = ''; return; }
  const chart = (serie) => {
    ...
    const width = 360; const height = 220; const left = 42; const right = 14; const top = 26; const bottom = 38; const base = height - bottom;
    ...
    const dots = values.map((value, index) => Number.isFinite(value) ? `<circle class="yield-point" cx="${x(index)}" cy="${y(value)}" r="3.5"><title>${escapeHtml(buckets[index])}: ${value.toFixed(2)}%</title></circle>` : '').join('');
    const labels = buckets.map((month, index) => `<text class="axis" x="${x(index)}" y="${base + 20}" text-anchor="middle">${escapeHtml(taYieldInterval === 'day' ? month.slice(5) : taYieldInterval === 'week' ? month.slice(-3) : month.slice(5))}</text>`).join('');
    return `<article class="sc-yield-series-card"><h4>%TTL Yield of ${escapeHtml(serie)}</h4><div class="sc-yield-chart-scroll"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="TA total yield for ${escapeHtml(serie)}"><text class="axis" x="4" y="${top + 4}">${max.toFixed(1)}%</text><text class="axis" x="4" y="${base + 4}">${min.toFixed(1)}%</text><line class="gridline" x1="${left}" y1="${top}" x2="${width - right}" y2="${top}"/><line class="gridline" x1="${left}" y1="${base}" x2="${width - right}" y2="${base}"/><polyline class="yield-line" points="${points}"/>${dots}${labels}</svg></div></article>`;
  };
  holder.innerHTML = series.map(chart).join('');
}

function renderTaYieldTendencyCharts(rows = latestTaYieldTendencyData) {
  latestTaYieldTendencyData = rows;
  const holder = byId('taYieldChart');
  holder.innerHTML = `<div class="table-heading"><div><p class="section-kicker">Total quality trend</p><h3>Yield and defect tendency</h3></div><label class="yield-interval-control" for="taYieldInterval">Group by<select id="taYieldInterval"><option value="day" ${taYieldInterval === 'day' ? 'selected' : ''}>Day</option><option value="week" ${taYieldInterval === 'week' ? 'selected' : ''}>Week</option><option value="month" ${taYieldInterval === 'month' ? 'selected' : ''}>Month</option></select></label></div><section class="sc-yield-series-section" aria-labelledby="taYieldGroupTrendTitle"><div><p class="section-kicker">Product group trend</p><h4 id="taYieldGroupTrendTitle">TTL Yield by product group</h4></div><div id="taYieldGroupYieldCharts" class="sc-yield-series-grid"></div></section><section class="ta-yield-tendency-panel"><h4>Total yield</h4><div id="taYieldYieldChart"></div></section><section class="ta-yield-tendency-panel"><h4>Defect rate by mode group</h4><div id="taYieldDefectChart"></div></section>`;
  ...
  const buckets = [...new Set(rows.map((row) => row.month))].sort().map((month) => rows.filter((row) => row.month === month).reduce((total, row) => ({ month, input: total.input + Number(row.input || 0), finalGood: total.finalGood + Number(row.finalGood || 0), groups: row.groups.reduce((values, group) => ({ ...values, [group.group]: (values[group.group] || 0) + Number(group.quantity || 0) }), total.groups) }), { month, input: 0, finalGood: 0, groups: {} })).map((row) => ({ ...row, yield: row.input ? row.finalGood / row.input * 100 : undefined }));
  renderTaYieldGroupTendencyCharts(rows, buckets.map((row) => row.month));
  ...
  const width = Math.max(760, buckets.length * 86 + 116); const height = 250; const left = 54; const right = 28; const top = 30; const bottom = 48; const base = height - bottom; const plotHeight = base - top; const slot = (width - left - right) / buckets.length; const label = (value) => taYieldInterval === 'day' ? value.slice(5) : taYieldInterval === 'week' ? value.slice(-3) : value.slice(5);
  const yields = buckets.map((row) => row.yield).filter(Number.isFinite); let minimum = Math.max(0, Math.floor((Math.min(...yields) - .5) * 2) / 2); let maximum = Math.min(100, Math.ceil((Math.max(...yields) + .5) * 2) / 2); if (maximum - minimum < 1) { minimum = Math.max(0, minimum - .5); maximum = Math.min(100, maximum + .5); }
  ...
  const targetsByBucket = new Map([...new Set(rows.map((row) => row.month))].map((month) => { const matching = rows.filter((row) => row.month === month && Number(row.input || 0) > 0); const targeted = matching.map((row) => ({ input: Number(row.input || 0), target: Number(taYieldTargets[row.line]?.[String(month).slice(0, 7)]) })).filter((row) => Number.isFinite(row.target)); const input = targeted.reduce((sum, row) => sum + row.input, 0); return [month, matching.length && targeted.length === matching.length ? targeted.reduce((sum, row) => sum + row.input * row.target, 0) / input : undefined]; })); const yieldPoints = buckets.map((row, index) => Number.isFinite(row.yield) ? `${x(index)},${yieldY(row.yield)}` : '').filter(Boolean).join(' '); const yieldDots = buckets.map((row, index) => Number.isFinite(row.yield) ? `<circle class="yield-point" cx="${x(index)}" cy="${yieldY(row.yield)}" r="4"><title>${escapeHtml(row.month)}: yield ${row.yield.toFixed(2)}%</title></circle>` : '').join(''); const labels = buckets.map((row, index) => `<text class="axis" x="${x(index)}" y="${base + 22}" text-anchor="middle">${escapeHtml(label(row.month))}</text>`).join(''); const yieldColumns = buckets.map((row, index) => { if (!Number.isFinite(row.yield)) return ''; const widthValue = Math.min(48, slot * .62); const heightValue = base - yieldY(row.yield); const target = targetsByBucket.get(row.month); const belowTarget = Number.isFinite(target) && row.yield < target; return `<rect class="yield-column${belowTarget ? ' below-target' : ''}" x="${x(index) - widthValue / 2}" y="${yieldY(row.yield)}" width="${widthValue}" height="${heightValue}"><title>${escapeHtml(row.month)}: yield ${row.yield.toFixed(2)}%${Number.isFinite(target) ? `; target ${target.toFixed(2)}%` : '; target incomplete'}</title></rect>`; }).join('');
  byId('taYieldYieldChart').innerHTML = `<div class="sc-yield-legend"><span><i class="yield-column-key"></i>Yield column</span><span><i class="yield-line-key"></i>Yield line</span></div><div class="sc-yield-chart-scroll"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="TA total yield by ${taYieldInterval}"><text class="axis axis-title" x="${left}" y="18">%Yield</text>${yieldGrid}<line x1="${left}" y1="${base}" x2="${width - right}" y2="${base}" stroke="#b8c7bf"/>${yieldColumns}<polyline class="yield-line" points="${yieldPoints}"/>${yieldDots}${labels}</svg></div>`;
  const defectRates = buckets.map((row) => row.input ? groups.reduce((total, group) => total + (row.groups[group] || 0), 0) / row.input * 100 : 0); const defectMaximum = Math.max(1, Math.ceil(Math.max(...defectRates) * 10) / 10); const defectGrid = [0, .5, 1].map((ratio) => { const y = base - plotHeight * ratio; return `<line class="gridline" x1="${left}" y1="${y}" x2="${width - right}" y2="${y}"/><text class="axis" x="${left - 8}" y="${y + 4}" text-anchor="end">${(defectMaximum * ratio).toFixed(1)}%</text>`; }).join('');
  const bars = buckets.map((row, index) => { const barWidth = Math.min(48, slot * .62); const barX = x(index) - barWidth / 2; let stacked = 0; return groups.map((group, groupIndex) => { const rate = row.input ? (row.groups[group] || 0) / row.input * 100 : 0; const barHeight = rate / defectMaximum * plotHeight; const y = base - stacked - barHeight; stacked += barHeight; return rate ? `<rect x="${barX}" y="${y}" width="${barWidth}" height="${barHeight}" fill="${chartColors[groupIndex % chartColors.length]}"><title>${escapeHtml(row.month)} | ${escapeHtml(group)}: ${rate.toFixed(3)}%</title></rect>` : ''; }).join(''); }).join(''); const legend = groups.map((group, index) => `<span><i style="background:${chartColors[index % chartColors.length]}"></i>${escapeHtml(group)}</span>`).join('');
  byId('taYieldDefectChart').innerHTML = `<div class="sc-yield-legend"><strong>Mode group</strong>${legend}</div><div class="sc-yield-chart-scroll"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="TA defect rate by ${taYieldInterval}"><text class="axis axis-title" x="${left}" y="18">%Defect</text>${defectGrid}<line x1="${left}" y1="${base}" x2="${width - right}" y2="${base}" stroke="#b8c7bf"/>${bars}${labels}</svg></div>`;
}
async function renderTaYieldTargetParameters() {
  const holder = byId('parameterView');
  const rows = latestTaYieldData.summary || [];
  const months = [...new Set(rows.map((row) => String(row.month).slice(0, 7)).filter((month) => /^\d{4}-\d{2}$/.test(month)))].sort();
  const series = [...new Set(rows.map((row) => taChartGroup(row.line)).filter(Boolean))].sort();
  try { const targets = await request('/api/ta-yield-targets'); taYieldTargets = targets.reduce((settings, target) => ({ ...settings, [target.serie]: { ...(settings[target.serie] || {}), [target.period]: target.target } }), {}); } catch (error) { holder.innerHTML = `<p class="parameter-status">${escapeHtml(error.message)}</p>`; return; }
  holder.innerHTML = `<div class="parameter-heading"><p class="section-kicker">Configuration</p><h2>TA Yield target parameters</h2><p>Set a monthly yield target for each TA series. Targets are stored separately on svr120a.</p></div><div class="table-wrap"><table><thead><tr><th>Series</th>${months.map((month) => `<th>${escapeHtml(month)}</th>`).join('')}</tr></thead><tbody>${series.map((serie) => `<tr><td>${escapeHtml(serie)}</td>${months.map((month) => `<td><input class="ta-yield-target-input" type="number" min="0" max="100" step="0.01" data-serie="${escapeHtml(serie)}" data-period="${month}" value="${taYieldTargets[serie]?.[month] ?? ''}" placeholder="Target" aria-label="${escapeHtml(serie)} yield target for ${month}" /></td>`).join('')}</tr>`).join('')}</tbody></table></div><p id="taYieldTargetStatus" class="parameter-status" role="status"></p>`;
  holder.querySelectorAll('.ta-yield-target-input').forEach((input) => input.addEventListener('change', async () => { const target = Number(input.value); const { serie, period } = input.dataset; const status = byId('taYieldTargetStatus'); if (input.value !== '' && (!Number.isFinite(target) || target < 0 || target > 100)) { input.setCustomValidity('Enter a target from 0 to 100.'); input.reportValidity(); return; } try { if (input.value === '') await request(`/api/ta-yield-targets?${new URLSearchParams({ serie, period })}`, { method: 'DELETE' }); else await request('/api/ta-yield-targets', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ serie, period, target }) }); status.textContent = 'Targets saved.'; } catch (error) { status.textContent = error.message; } }));
}
async function renderTaYieldTargetParameters() {
  const holder = byId('parameterView');
  const series = [...new Set((latestTaYieldData.summary || []).map((row) => row.line).filter(Boolean))].sort();
  ...
  holder.innerHTML = `<div class="parameter-heading"><p class="section-kicker">Configuration</p><h2>TA Yield target parameters</h2><p>Set a monthly yield target for a TA series. This date selector is independent from the dashboard reporting range.</p></div><form id="taYieldTargetForm" class="parameter-form"><label>Serie<select id="taYieldTargetSerie" required><option value="">Select serie</option>${series.map((serie) => `<option value="${escapeHtml(serie)}">${escapeHtml(serie)}</option>`).join('')}</select></label><label>Period<input id="taYieldTargetPeriod" type="month" value="${period}" required /></label><label>Yield target (%)<input id="taYieldTargetValue" type="number" min="0" max="100" step="0.01" required /></label><button type="submit">Save target</button></form><p id="taYieldTargetStatus" class="parameter-status" role="status"></p><div class="saved-parameters"><h3>Saved TA Yield targets</h3><div class="table-wrap"><table><thead><tr><th>Serie</th><th>Period</th><th>Yield target</th><th>Action</th></tr></thead><tbody>${targets.map((target) => `<tr><td>${escapeHtml(target.serie)}</td><td>${escapeHtml(target.period)}</td><td>${Number(target.target).toFixed(2)}%</td><td><button class="remove-ta-yield-target" data-serie="${escapeHtml(target.serie)}" data-period="${target.period}" type="button">Remove</button></td></tr>`).join('') || '<tr><td colspan="4">No TA Yield targets saved.</td></tr>'}</tbody></table></div></div>`;
  ...
  holder.querySelectorAll('.remove-ta-yield-target').forEach((button) => button.addEventListener('click', async () => { await request(`/api/ta-yield-targets?${new URLSearchParams(button.dataset)}`, { method: 'DELETE' }); await renderTaYieldTargetParameters(); renderTaYieldTendencyCharts(); }));
}
function ensureTaWorkbookVerificationView() {
  let button = document.querySelector('[data-ta-yield-table="workbook"]');
  if (!button) { button = document.createElement('button'); button.type = 'button'; button.dataset.taYieldTable = 'workbook'; button.textContent = 'DataTable'; }
  button.className = 'app-tab'; button.dataset.view = 'ta-workbook'; button.setAttribute('aria-pressed', 'false');
  document.querySelector('.app-tabs').append(button);
  let holder = byId('taWorkbookVerification');
  if (!holder) { holder = document.createElement('section'); holder.id = 'taWorkbookVerification'; holder.className = 'sc-yield-series-section'; holder.hidden = true; holder.innerHTML = '<div class="table-heading"><div><p class="section-kicker">Reference reconciliation</p><h3>Workbook-format verification</h3></div><span id="taWorkbookCount" role="status"></span></div><div class="ta-yield-lot-filter"><label>Line<select id="taWorkbookLine"></select></label><label>Category<select id="taWorkbookCategory"></select></label><label>Find lot/item<input id="taWorkbookSearch" type="search" /></label></div><div class="table-wrap"><table><thead><tr><th>ProdLine</th><th>JobName</th><th>From ItemName</th><th>Taping Date</th><th>Category</th><th>Quantity</th></tr></thead><tbody id="taWorkbookRows"></tbody></table></div>'; byId('taYieldSection').append(holder); }
  const render = () => { const line = byId('taWorkbookLine').value; const category = byId('taWorkbookCategory').value; const search = byId('taWorkbookSearch').value.trim().toLowerCase(); const categories = [...new Set(latestTaWorkbookRows.flatMap((row) => Object.keys(row.categories)))].sort(); const rows = latestTaWorkbookRows.filter((row) => (!line || row.line === line) && (!category || row.categories[category]) && (!search || `${row.lotNo} ${row.itemName}`.toLowerCase().includes(search))); byId('taWorkbookCount').textContent = `${rows.length} of ${latestTaWorkbookRows.length} lots`; holder.querySelector('thead').innerHTML = `<tr><th>ProdLine</th><th>JobName</th><th>From ItemName</th><th>Taping Date</th>${categories.map((value) => `<th>${escapeHtml(value)}</th>`).join('')}</tr>`; byId('taWorkbookRows').innerHTML = rows.map((row) => `<tr><td>${escapeHtml(row.line)}</td><td>${escapeHtml(row.lotNo)}</td><td>${escapeHtml(row.itemName)}</td><td>${escapeHtml(bangkokDate(row.tapingDate))}</td>${categories.map((value) => `<td>${row.categories[value] ? format.format(row.categories[value]) : ''}</td>`).join('')}</tr>`).join('') || `<tr><td colspan="${4 + categories.length}">No workbook rows match these filters.</td></tr>`; };
  const load = async () => { const requestId = dataRequestId; const params = new URLSearchParams({ dataset: 'ta-yield', startDate: byId('startDate').value, endDate: byId('endDate').value }); const rows = await request(`/api/ta-yield-workbook-reconciliation?${params}`); if (requestId !== dataRequestId) return; latestTaWorkbookRows = rows; const lines = [...new Set(latestTaWorkbookRows.map((row) => row.line))].sort(); const categories = [...new Set(latestTaWorkbookRows.flatMap((row) => Object.keys(row.categories)))].sort(); byId('taWorkbookLine').replaceChildren(new Option('All lines', ''), ...lines.map((value) => new Option(value, value))); byId('taWorkbookCategory').replaceChildren(new Option('All categories', ''), ...categories.map((value) => new Option(value, value))); render(); };
  holder.loadRows = load;
  if (!button.dataset.bound) { button.dataset.bound = 'true'; button.addEventListener('click', async () => { taYieldTableView = 'workbook'; document.querySelectorAll('.app-tab').forEach((item) => { const active = item === button; item.classList.toggle('active', active); item.setAttribute('aria-pressed', String(active)); }); holder.hidden = false; byId('taYieldLotFilter').hidden = true; document.querySelector('#taYieldSection > .sc-yield-series-section[aria-labelledby="taYieldDetailTitle"]').hidden = true; try { await load(); } catch (error) { byId('taWorkbookRows').innerHTML = `<tr><td colspan="6">${escapeHtml(error.message)}</td></tr>`; } }); byId('taWorkbookLine').addEventListener('change', render); byId('taWorkbookCategory').addEventListener('change', render); byId('taWorkbookSearch').addEventListener('input', render); }
  if (!button.dataset.viewBound) { button.dataset.viewBound = 'true'; button.addEventListener('click', () => { [...byId('taYieldSection').children].forEach((child) => { if (child !== holder) child.hidden = true; }); holder.hidden = false; }); }
  ...
  if (!holder.dataset.tableBound) { holder.dataset.tableBound = 'true'; new MutationObserver(applyWorkbookLimit).observe(byId('taWorkbookRows'), { childList: true }); holder.addEventListener('click', (event) => { if (event.target.closest('th')?.cellIndex !== 3) return; taWorkbookDateDirection = taWorkbookDateDirection === 'asc' ? 'desc' : 'asc'; const rows = [...byId('taWorkbookRows').querySelectorAll('tr')].sort((left, right) => { const comparison = left.cells[3].textContent.localeCompare(right.cells[3].textContent); return taWorkbookDateDirection === 'asc' ? comparison : -comparison; }); byId('taWorkbookRows').append(...rows); applyWorkbookLimit(); }); }
  applyWorkbookLimit();
  return holder;
}
function renderTaYield(payload) {
  byId('taWorkbookStaticControl').hidden = false;
  const workbookHolder = ensureTaWorkbookVerificationView();
  workbookHolder.hidden = taYieldTableView !== 'workbook';
  byId('taYieldHead').closest('section').hidden = taYieldTableView === 'workbook';
  if (taYieldTableView === 'workbook') workbookHolder.loadRows().catch(() => {});
  ...
  lotSeriesSelect.replaceChildren(new Option('Select a series', ''), ...lotSeries.map((series) => new Option(series, series)));
  ...
  if (taYieldTableView === 'lots') {
    ...
    byId('taYieldRows').innerHTML = rows.map((row) => { const quantities = Object.fromEntries(row.groups.map((group) => [group.group, group.quantity])); return `<tr><td>${escapeHtml(row.month)}</td><td>${escapeHtml(row.line)}</td><td>${format.format(row.input)}</td><td>${format.format(row.finalGood)}</td>${groups.map((group) => `<td>${format.format(quantities[group] || 0)}</td>`).join('')}<td>${row.defectRate === undefined ? '-' : `${row.defectRate.toFixed(2)}%`}</td><td>${row.yield === undefined ? '-' : `${row.yield.toFixed(2)}%`}</td></tr>`; }).join('') || `<tr><td colspan="${6 + groups.length}">No eligible TA Yield data matches the selected filters.</td></tr>`;
  }
  ...
  const bars = chartRows.map((row, index) => { const x = left + index * slot + slot * .28; const barWidth = Math.min(42, slot * .44); const quantities = Object.fromEntries(row.groups.map((group) => [group.group, group.quantity])); let stacked = 0; const segments = groups.map((group, groupIndex) => { const rate = row.input ? (quantities[group] || 0) / row.input * 100 : 0; const h = rate / defectMax * plotHeight; const y = base - stacked - h; stacked += h; return rate ? `<rect x="${x}" y="${y}" width="${barWidth}" height="${h}" fill="${chartColors[groupIndex % chartColors.length]}"><title>${escapeHtml(row.month)} ${escapeHtml(row.line)} | ${escapeHtml(group)}: ${rate.toFixed(3)}%</title></rect>` : ''; }).join(''); return `${segments}<text class="axis" x="${x + barWidth / 2}" y="${base + 20}" text-anchor="middle">${escapeHtml(row.line)}</text><text class="axis" x="${x + barWidth / 2}" y="${base + 35}" text-anchor="middle">${escapeHtml(row.month.slice(5))}</text>`; }).join('');
...
async function loadData() {
  ...
  const period = selectedReportingPeriod(); const isLot = currentConfig.dataset === 'lot'; const isScYield = currentConfig.dataset === 'yield'; const isTaYield = currentConfig.dataset === 'ta-yield';
  ...
  try {
    ...
    if (isTaYield) { const tendencyParams = new URLSearchParams(params); tendencyParams.set('interval', taYieldInterval); const [summary, tendencyRows, weeklyRows, targets] = await Promise.all([request(`/api/ta-yield?${params}`), request(`/api/ta-yield-tendency?${tendencyParams}`), request(`/api/ta-yield-weekly?${params}`), request('/api/ta-yield-targets')]); if (requestId !== dataRequestId) return; taYieldTargets = targets.reduce((settings, target) => ({ ...settings, [target.serie]: { ...(settings[target.serie] || {}), [target.period]: target.target } }), {}); latestTaYieldLotsUrl = `/api/ta-yield-lots?${params}`; latestTaYieldData = { summary, details: [] }; latestTaYieldTendencyData = tendencyRows; latestTaYieldWeeklyData = weeklyRows; if (taYieldTableView === 'lots') { const details = await request(latestTaYieldLotsUrl); if (requestId !== dataRequestId) return; latestTaYieldData = { ...latestTaYieldData, details }; } renderTaYield(latestTaYieldData); renderTaYieldWeeklyChart(weeklyRows); setStatus(''); return; }
    const [data, chartData, mtdData, wipFlow, yieldData] = await Promise.all([request(`/api/quantity?${params}`), currentConfig.chartAxis === 'process' && !usesOperationDateAxis() ? request(`/api/chart?${params}`) : Promise.resolve(null), mtdParams ? request(`/api/mtd-quantity?${mtdParams}`) : Promise.resolve([]), isLot ? request(`/api/wip-flow?${params}`) : Promise.resolve([]), isLot ? request(`/api/yield?${params}`) : Promise.resolve({ goodDisposition: 'good', rows: [] }), loadCellComments()]);
    if (requestId !== dataRequestId) return; latestWipFlow = wipFlow; latestYieldData = yieldData; renderData(data, chartData, mtdData); renderWipInsights(wipFlow, yieldData); setStatus('');
  } catch (error) { if (requestId === dataRequestId) setStatus(error.message); } finally { setReportControlsLoading(false); if (requestId === dataRequestId) apply.textContent = 'Apply'; }
}
...
document.addEventListener('click', (event) => { const tab = event.target.closest('.app-tab'); if (tab?.dataset.view && tab.dataset.view !== 'ta-workbook') { byId('taWorkbookVerification')?.setAttribute('hidden', ''); byId('taYieldHead')?.closest('section')?.removeAttribute('hidden'); } });
document.addEventListener('click', (event) => { if (event.target.closest('.app-tab')?.dataset.view === 'dashboard' && selectedDataset() === 'ta-yield') { [...byId('taYieldSection').children].forEach((child) => { if (child.id !== 'taWorkbookVerification') child.hidden = false; }); byId('taYieldLotFilter').hidden = taYieldTableView !== 'lots'; } });
...
byId('toggleWipFlow').addEventListener('change', () => setWipInsightVisibility({ flow: byId('toggleWipFlow').checked }));
byId('toggleYield').addEventListener('change', () => setWipInsightVisibility({ yield: byId('toggleYield').checked }));
document.querySelectorAll('.process-option').forEach((button) => button.addEventListener('click', () => { setSelectedProduct(byId('product').value === button.dataset.product ? '' : button.dataset.product); refreshOptionsForProduct(); }));
...
byId('dataSource').addEventListener('change', () => { const isScYield = selectedDataset() === 'yield'; const isTaYield = selectedDataset() === 'ta-yield'; byId('scYieldLogTab').hidden = !isScYield; byId('taYieldLogTab').hidden = !isTaYield; const activeView = document.querySelector('.app-tab.active')?.dataset.view; if ((!isScYield && activeView === 'sc-yield-log') || (!isTaYield && activeView === 'ta-yield-log')) showView('dashboard'); });
