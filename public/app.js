...
let scYieldTargetPendingSaves = {};
let scYieldTargetSettingsPromise;
function loadScYieldTargetSettings() {
  if (!scYieldTargetSettingsPromise) scYieldTargetSettingsPromise = fetchScYieldTargetSettings().finally(() => { scYieldTargetSettingsPromise = undefined; });
  return scYieldTargetSettingsPromise;
}
async function fetchScYieldTargetSettings() { if (!scYieldTargetStorageRemote) return; const local = readScYieldTargetSettings(); let rows = await request('/api/sc-yield-targets'); const stored = new Set(rows.map((row) => `${row.serie}|${row.period}`)); const missing = Object.entries(local).flatMap(([serie, periods]) => Object.entries(periods || {}).filter(([, target]) => Number.isFinite(Number(target))).map(([period, target]) => ({ serie, period, target: Number(target) }))).filter((target) => !stored.has(`${target.serie}|${target.period}`)); if (missing.length) { await Promise.all(missing.map((target) => request('/api/sc-yield-targets', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(target) }))); rows = await request('/api/sc-yield-targets'); } scYieldTargetSettings = rows.reduce((settings, row) => ({ ...settings, [row.serie]: { ...(settings[row.serie] || {}), [row.period]: row.target } }), {}); localStorage.removeItem(scYieldTargetSettingsKey); scYieldTargetSettingsLoaded = true; }
function scYieldTargetFor(serie, month) {
  const saved = scYieldTargetSettings?.[serie]?.[month];
  if (typeof saved !== 'number' && typeof saved !== 'string') return undefined;
  if (typeof saved === 'string' && !saved.trim()) return undefined;
  const value = Number(saved);
  return Number.isFinite(value) && value >= 0 && value <= 100 ? value : undefined;
}
async function renderScYieldTargetParameters(rows = latestScYieldData) {
  const period = byId('scYieldTargetPeriod');
  if (!period.value) period.value = bangkokToday().slice(0, 7);
  if (scYieldTargetStorageRemote && !scYieldTargetSettingsLoaded) await loadScYieldTargetSettings();
  const month = period.value;
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(month)) return;
  const knownSeries = [...new Set([...rows.map((row) => row.line || 'Unspecified'), ...Object.keys(scYieldTargetSettings)])];
  const series = ['Total', ...knownSeries.filter((serie) => serie !== 'Total').sort()];
  byId('scYieldTargetParameters').innerHTML = `<table><thead><tr><th scope="col">Target scope</th><th scope="col">${escapeHtml(month)} target (%)</th></tr></thead><tbody>${series.map((serie) => {
    const pending = scYieldTargetPendingSaves[JSON.stringify([serie, month])];
    const value = pending ? pending.value : scYieldTargetFor(serie, month);
    return `<tr><th scope="row">${escapeHtml(serie)}</th><td><input class="sc-yield-series-target-input" type="number" min="0" max="100" step="0.01" inputmode="decimal" data-sc-yield-target-serie="${escapeHtml(serie)}" data-sc-yield-target-month="${month}" value="${escapeHtml(value ?? '')}"${pending ? ' disabled' : ''} placeholder="No target" aria-label="${escapeHtml(serie)} yield target for ${month}" /></td></tr>`;
  }).join('')}</tbody></table>`;
}
function finishScYieldTargetSave(serie, period) {
  const key = JSON.stringify([serie, period]);
  scYieldTargetPendingSaves = Object.fromEntries(Object.entries(scYieldTargetPendingSaves).filter(([pendingKey]) => pendingKey !== key));
  byId('scYieldTargetParameters').querySelectorAll('.sc-yield-series-target-input').forEach((field) => {
    if (field.dataset.scYieldTargetSerie !== serie || field.dataset.scYieldTargetMonth !== period) return;
    field.value = String(scYieldTargetFor(serie, period) ?? '');
    field.disabled = false;
  });
}
async function saveScYieldTarget(input) {
  const serie = input.dataset.scYieldTargetSerie;
  const period = input.dataset.scYieldTargetMonth;
  const empty = input.value.trim() === '';
  const target = Number(input.value);
  const status = byId('scYieldTargetStatus');
  const key = JSON.stringify([serie, period]);
  if (Object.hasOwn(scYieldTargetPendingSaves, key)) return;
  input.setCustomValidity('');
  if (!serie?.trim() || !/^\d{4}-(0[1-9]|1[0-2])$/.test(period) || input.validity?.badInput || input.validity?.stepMismatch || (!empty && (!Number.isFinite(target) || target < 0 || target > 100))) {
    input.setCustomValidity('Enter a target from 0 to 100 for a valid month.');
    input.reportValidity();
    return;
  }
  scYieldTargetPendingSaves = { ...scYieldTargetPendingSaves, [key]: { value: input.value } };
  input.disabled = true;
  status.className = 'parameter-status';
  status.textContent = `Saving ${serie} target for ${period}...`;
  try {
    if (scYieldTargetStorageRemote) {
      if (empty) await request(`/api/sc-yield-targets?${new URLSearchParams({ serie, period })}`, { method: 'DELETE' });
      else await request('/api/sc-yield-targets', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ serie, period, target }) });
    }
    const periods = scYieldTargetSettings[serie] || {};
    const nextPeriods = empty ? Object.fromEntries(Object.entries(periods).filter(([month]) => month !== period)) : { ...periods, [period]: target };
    const next = { ...scYieldTargetSettings, [serie]: nextPeriods };
    if (!scYieldTargetStorageRemote) localStorage.setItem(scYieldTargetSettingsKey, JSON.stringify(next));
    scYieldTargetSettings = next;
    status.className = 'parameter-status success';
    status.textContent = `${serie} target for ${period} ${empty ? 'removed' : 'saved'}.`;
    if (selectedDataset() === 'yield') renderScYield();
  } catch (error) {
    status.className = 'parameter-status';
    status.textContent = `Could not save ${serie} target for ${period}: ${error.message}`;
  } finally { input.disabled = false; finishScYieldTargetSave(serie, period); }
}
let pnSearchTimer;
let dataRequestId = 0;
const selectedDataset = () => byId('dataSource').value;
const dashboardSubtabOptions = {
...
const dashboardSubtabsKey = 'onemes-dashboard-subtabs-v1';
function readDashboardSubtabs() {
  let saved;
  try { saved = JSON.parse(sessionStorage.getItem(dashboardSubtabsKey) || 'null'); }
  catch (error) { console.warn('Dashboard subtabs could not be restored.', error); }
  return Object.fromEntries(Object.entries(dashboardSubtabOptions).map(([name, options]) =>
    [name, options.includes(saved?.[name]) ? saved[name] : options[0]]));
}
let dashboardSubtabs = readDashboardSubtabs();
function saveDashboardSubtab(name, value) {
  if (!Object.hasOwn(dashboardSubtabOptions, name) || !dashboardSubtabOptions[name].includes(value)) return;
  dashboardSubtabs = { ...dashboardSubtabs, [name]: value };
  try { sessionStorage.setItem(dashboardSubtabsKey, JSON.stringify(dashboardSubtabs)); }
  catch (error) { console.warn('Dashboard subtabs could not be saved.', error); }
  saveDashboardNavigation();
}
function restoreDashboardSubtabControls() {
  const controls = [
    ['.chart-mode', 'chartMode', 'productionChart'],
    ['.mtd-chart-mode', 'mtdChartStyle', 'mtdChart'],
    ['.ta-yield-table-mode', 'taYieldTable', 'taTable'],
  ];
  controls.forEach(([selector, attribute, name]) => document.querySelectorAll(selector).forEach((button) => {
    const active = button.dataset[attribute] === dashboardSubtabs[name];
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', String(active));
  }));
}
const dashboardNavigationKey = 'onemes-dashboard-navigation-v1';
let dashboardNavigationVersion = 0;
let restoredDashboardProduct = false;
function readDashboardNavigation() {
  try {
    const saved = JSON.parse(sessionStorage.getItem(dashboardNavigationKey) || 'null');
    if (!['closed', 'lot', 'yield', 'ta-yield'].includes(saved?.dataset)) return undefined;
    const views = ['dashboard', 'parameters', 'comments', 'model', 'defects', 'staging', 'ta-data-table', 'ta-yield-machine', 'sc-yield-log', 'ta-yield-log'];
    return { dataset: saved.dataset, view: views.includes(saved.view) ? saved.view : 'dashboard', product: ['NEO', 'SC', ''].includes(saved.product) ? saved.product : '' };
  } catch (error) {
    console.warn('Dashboard navigation could not be restored.', error);
    return undefined;
  }
}
function saveDashboardNavigation(view = document.querySelector('.app-tab.active')?.dataset.view || 'dashboard') {
  dashboardNavigationVersion += 1;
  try {
    sessionStorage.setItem(dashboardNavigationKey, JSON.stringify({ dataset: selectedDataset(), view, product: byId('product').value }));
  } catch (error) {
    console.warn('Dashboard navigation could not be saved.', error);
  }
}
async function initializeDashboardNavigation() {
  const saved = readDashboardNavigation();
  if (saved) {
    byId('dataSource').value = saved.dataset;
    setSelectedProduct(saved.product);
    restoredDashboardProduct = true;
  }
  updateYieldUtilityTabs();
  const version = dashboardNavigationVersion;
  const dataset = selectedDataset();
  await initialize();
  if (!saved || version !== dashboardNavigationVersion || dataset !== selectedDataset() || currentConfig.dataset !== dataset) return;
  const tabs = [...document.querySelectorAll('.app-tab')];
  const tab = tabs.find((button) => button.dataset.view === saved.view && !button.hidden)
    || tabs.find((button) => button.dataset.view === 'dashboard');
  tab?.click();
}
...
function taYieldTargetFor(serie, bucket) {
  ...
  const savedTarget = taYieldTargets[shortTaSeries(serie)]?.[period];
  if (typeof savedTarget !== 'number' && typeof savedTarget !== 'string') return undefined;
  if (typeof savedTarget === 'string' && !savedTarget.trim()) return undefined;
  const target = Number(savedTarget);
  return Number.isFinite(target) && target >= 0 && target <= 100 ? target : undefined;
}
function taYieldTargetSegments(values, x, y) {
  return values.map((value, index) => Number.isFinite(value) ? `${x(index)},${y(value)}` : '\n')
...
let scYieldActionStatus = dashboardSubtabs.scActions;
...
let taYieldActionStatus = dashboardSubtabs.taActions;
let taYieldDetailVisible = dashboardSubtabs.taDetails === 'visible';
...
let taYieldTargets = {};
let taYieldTargetTab = dashboardSubtabs.taTargets;
...
let savedParameterTab = dashboardSubtabs.mtdParameters;
function renderSavedParameters() {
  ...
  tabs.querySelectorAll('[data-saved-parameter-tab]').forEach((button) => button.addEventListener('click', () => { savedParameterTab = button.dataset.savedParameterTab; saveDashboardSubtab('mtdParameters', savedParameterTab); renderSavedParameters(); }));
...
const taMtdSeries = Object.freeze(['FPS', 'FPS A08', 'FPS A2', 'FPS A3', 'FPS B10', 'FPS B2', 'FPS B3', 'FPU A2', 'GPS', 'GPS P2', 'PSG', 'PSG B2', 'PSH B2', 'PSL', 'PSL A', 'PSL B15', 'PSL B2', 'PSL B3', 'PSU B2']);
async function loadParameterSeries() { const product = byId('parameterProduct').value; const serie = byId('parameterSerie'); if (!product) { serie.replaceChildren(new Option('Select product first', '')); serie.disabled = true; return; } byId('parameterStatus').textContent = ''; try { let values; if (product === 'TA') values = taMtdSeries; else { const options = await request(`/api/options?${new URLSearchParams({ dataset: selectedDataset(), product })}`); values = options.serie; } serie.replaceChildren(new Option('Select serie', ''), ...values.map((value) => new Option(value, value))); serie.disabled = false; const dashboardSelection = selectedSeries(); if (dashboardSelection.length === 1 && values.includes(dashboardSelection[0])) serie.value = dashboardSelection[0]; fillParameterSetting(); } catch (error) { byId('parameterStatus').textContent = error.message; } }
...
let stagingMonitorTimer;
let stagingMonitorTab = dashboardSubtabs.staging;
...
function renderStagingPipelineMap(rows) { const pipeline = (row) => { const stages = stagingPipelineBlueprint(row.name, row.source); const health = stagingHealth(row); return `<article class="staging-flow ${health}"><header><div><span class="staging-state ${health}">${escapeHtml(health)}</span><h3>${escapeHtml(row.name)}</h3></div><span class="staging-flow-rows">${row.activityAvailable ? `${format.format(row.rowCount || 0)} rows` : 'Activity unavailable'}</span></header><div class="staging-flow-stages">${stages.map((stage, index) => `<div class="staging-flow-stage ${stage.kind}"><span class="staging-flow-order">${index + 1}</span><b>${escapeHtml(stage.label)}</b><small>${escapeHtml(stage.detail)}</small></div>`).join('')}</div><footer><span>Coverage: ${escapeHtml(row.firstDataDate ? `${String(row.firstDataDate).slice(0, 10)} → ${String(row.lastDataDate).slice(0, 10)}` : 'Waiting for data')}</span><span>${escapeHtml(stagingTime(row.lastRefreshedAt))}</span></footer></article>`; }; return `<section class="staging-pipeline-map" aria-labelledby="stagingMapTitle"><header class="staging-map-heading"><div><p class="section-kicker">Data lineage</p><h2 id="stagingMapTitle">Staging pipeline map</h2><p>Follow each source through lookup, transformation, staging table, and validation. Stage colors show the role; each lane border shows current pipeline health.</p></div><div class="staging-map-legend" aria-label="Pipeline stage legend"><span class="source">Source</span><span class="lookup">Lookup</span><span class="transform">Transform</span><span class="destination">Stage table</span><span class="validate">Validate</span></div></header><div class="staging-flow-list">${rows.map(pipeline).join('')}</div></section>`; }
function setStagingMonitorTab(view, tab) { view.querySelectorAll('[data-staging-monitor-tab]').forEach((button) => { const active = button.dataset.stagingMonitorTab === tab; button.classList.toggle('active', active); button.setAttribute('aria-selected', String(active)); }); view.querySelectorAll('[data-staging-monitor-panel]').forEach((panel) => { panel.hidden = panel.dataset.stagingMonitorPanel !== tab; }); }
...
function renderScYieldArSummary(rates, rows) {
  ...
  const totalAr = Number.isFinite(latest.yield) && latest.target > 0 ? latest.yield / latest.target * 100 : undefined;
...
function renderScYield(rows = latestScYieldData) {
  ...
  const rates = byMonth.map((entry) => { const target = scYieldTargetFor('Total', entry.month); return { ...entry, target, defectRate: entry.input ? entry.defect / entry.input * 100 : undefined, yield: entry.input ? (entry.input - entry.defect) / entry.input * 100 : undefined, groupRates: Object.fromEntries(groups.map((group) => [group, entry.input ? (entry.groups[group] || 0) / entry.input * 100 : 0])) }; });
...
function renderScYieldTendencyCharts(rows = latestScYieldTendencyData, focusTargetId = '') {
  ...
  const buckets = bucketNames.map((month) => {
    ...
    const target = scYieldTargetFor(scYieldTrendSeries, taYieldTargetPeriod(month));
    return { month, input, defect, groups: grouped, target, yield: input ? (input - defect) / input * 100 : undefined };
  });
  ...
  const targetSegments = taYieldTargetSegments(buckets.map((row) => row.target), x, yieldY);
  const targetPoints = targetSegments.map((points) => `<polyline class="target-line" points="${points}"/>`).join('');
  ...
  byId('scYieldYieldChart').innerHTML = `<div class="sc-yield-legend">${yieldLegend}${targetPoints ? '<span><i class="target-line-key"></i>Target</span>' : ''}</div><div class="sc-yield-chart-scroll" tabindex="0" aria-label="Scrollable SC yield chart"${viewportStyle}><svg${svgStyle} viewBox="0 0 ${width} ${height}" role="group" aria-label="SC ${escapeHtml(scope)} yield by ${scYieldInterval}"><text class="axis axis-title" x="${left}" y="18">%Yield</text>${yieldGrid}<line x1="${left}" y1="${base}" x2="${width - right}" y2="${base}" stroke="#b8c7bf"/>${yieldVisual}${targetPoints}${targetDots}${labels}</svg></div>`;
...
function renderTaYieldGroupTendencyCharts(rows, buckets) {
  ...
  const chart = (serie) => {
    const pointsByMonth = buckets.map((month) => {
      ...
      return { yield: input ? finalGood / input * 100 : undefined, target: taYieldTargetFor(serie, month) };
    });
    ...
    const targetPoints = taYieldTargetSegments(pointsByMonth.map((point) => point.target), x, y).map((points) => `<polyline class="target-line" points="${points}"/>`).join('');
...
function renderTaYieldTargetSavedList(targets) {
  ...
  document.querySelectorAll('[data-ta-yield-target-tab]').forEach((button) => { const active = button.dataset.taYieldTargetTab === taYieldTargetTab; button.classList.toggle('active', active); button.setAttribute('aria-selected', String(active)); button.tabIndex = active ? 0 : -1; if (active) byId('taYieldTargetGroups').setAttribute('aria-labelledby', button.id); });
...
async function renderTaYieldTargetParameters() {
  ...
  const fixedSeries = ['Total', 'Facedown', 'Standard Production', 'GPS'];
  ...
  holder.addEventListener('click', async (event) => { const tab = event.target.closest('[data-ta-yield-target-tab]'); if (tab) { taYieldTargetTab = tab.dataset.taYieldTargetTab; saveDashboardSubtab('taTargets', taYieldTargetTab); byId('taYieldTargetGroups').setAttribute('aria-labelledby', tab.id); renderTaYieldTargetSavedList(targets); return; } const remove = event.target.closest('.remove-ta-yield-target'); if (!remove) return; await request(`/api/ta-yield-targets?${new URLSearchParams(remove.dataset)}`, { method: 'DELETE' }); await renderTaYieldTargetParameters(); renderTaYieldTendencyCharts(); });
  holder.addEventListener('keydown', (event) => { const tab = event.target.closest('[data-ta-yield-target-tab]'); if (!tab || !['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return; const tabs = [...holder.querySelectorAll('[data-ta-yield-target-tab]')]; const current = tabs.indexOf(tab); const nextIndex = event.key === 'Home' ? 0 : event.key === 'End' ? tabs.length - 1 : (current + (event.key === 'ArrowRight' ? 1 : -1) + tabs.length) % tabs.length; const next = tabs[nextIndex]; event.preventDefault(); taYieldTargetTab = next.dataset.taYieldTargetTab; saveDashboardSubtab('taTargets', taYieldTargetTab); byId('taYieldTargetGroups').setAttribute('aria-labelledby', next.id); renderTaYieldTargetSavedList(targets); next.focus(); });
...
document.querySelectorAll('.mtd-chart-mode').forEach((button) => button.addEventListener('click', () => { mtdChartStyle = button.dataset.mtdChartStyle; saveDashboardSubtab('mtdChart', mtdChartStyle); document.querySelectorAll('.mtd-chart-mode').forEach((mode) => { const active = mode === button; mode.classList.toggle('active', active); mode.setAttribute('aria-pressed', String(active)); }); renderMtd(latestMtdData); }));
...
byId('scYieldTargetParameters').addEventListener('change', (event) => { if (event.target.matches('.sc-yield-series-target-input')) saveScYieldTarget(event.target); });
byId('scYieldTargetPeriod').addEventListener('change', () => {
  const period = byId('scYieldTargetPeriod');
  period.setCustomValidity('');
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(period.value)) { period.setCustomValidity('Choose a target month.'); period.reportValidity(); return; }
  byId('scYieldTargetStatus').textContent = '';
  renderScYieldTargetParameters(latestScYieldData).catch((error) => { byId('scYieldTargetStatus').className = 'parameter-status'; byId('scYieldTargetStatus').textContent = error.message; });
});
...
function renderTaYieldMultiSeriesChart(rows, buckets, targetsByBucket, label, isTotal, targetLabel, aggregateLabel) {
  ...
  const targetPlot = taYieldTargetSegments(target, x, y).map((points) => `<polyline points="${points}" fill="none" stroke="#ffffff" stroke-width="9"/><polyline class="ta-yield-total-target-line" points="${points}" fill="none" stroke="#f15a24" stroke-width="4" stroke-dasharray="9 5"/>`).join('') + `${marks(target, '#f15a24', targetLabel, 4)}`;
