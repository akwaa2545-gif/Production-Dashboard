...
const stagingHealth = (row, now = Date.now()) => !row.enabled ? 'disabled' : row.activityError ? 'failed' : !row.activityAvailable || !row.lastRefreshedAt ? 'waiting' : now - new Date(row.lastRefreshedAt).getTime() > row.intervalMs * 2 ? 'stale' : 'healthy';
