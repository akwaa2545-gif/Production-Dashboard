...
refreshWipStaging({ source: new SqlRepository(sourceConfig), target: new StagingWipRepository(targetConfig), targetConfig, startDate, endDate })
  .then((result) => {
    console.log(`WIP staging refreshed: ${result.rows} rows for ${result.startDate} to ${result.endDate}.`);
    process.exit(0);
  })
  .catch((error) => {
    console.error(`WIP staging refresh failed: ${error.message}`);
    process.exit(1);
  });
