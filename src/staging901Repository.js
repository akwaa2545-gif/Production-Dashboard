...
const filtersWhere = (request, filters) => {
  ...
  for (const [key, column] of [['serie', 'Serie'], ['pn', 'PartNumber']]) {
    ...
    if (values.length) { request.input(key, sql.NVarChar(sql.MAX), JSON.stringify(values)); clauses.push(`${column} IN (SELECT [value] FROM OPENJSON(@${key}))`); }
  }
...
export class Staging901Repository {
  ...
  async getOptions(filters = {}) { const request = (await this.getPool()).request(); const where = filtersWhere(request, { ...filters, startDate: filters.startDate || '2000-01-01', endDate: filters.endDate || '2100-01-01' }); const result = await request.query(`SELECT DISTINCT Product AS process, Product AS product, Serie AS serie FROM ${quoted(this.config.table)} WHERE ${where} ORDER BY product, serie`); return result.recordset; }
