...
const thailandDate = (value) => value instanceof Date
  ? new Date(value.getTime() + 7 * 60 * 60 * 1000).toISOString().slice(0, 10)
...
const calculatedLot = (lot) => {
  if (lot.jobType === 'NON-STANDARD' || lot.jobType === 'E' || !validLot(lot.lotNo)) return undefined;
...
export function mapTaWorkbookReconciliationRows(rows, categories) {
  ...
  rows.forEach((row) => {
    ...
    const key = `${row.line}|${row.lotNo}|${row.itemName}|${String(row.tapingDate)}`;
